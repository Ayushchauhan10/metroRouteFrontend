    ...
    return (
        ...
        <div className='w-full flex flex-col items-center justify-center gap-2'>
            <div className='flex flex-col items-center justify-center gap-2'>
              <div className='text-[1.3rem] font-bold text-violet-500 '>Welcome!!! </div>
        ...
        <div className='flex flex-col items-start justify-center  md:w-[70%] w-[95%] gap-3 '>
            ...
            <div className='flex mb-20 md:mb-0 md:flex-row flex-col h-[600px] md:h-auto overflow-y-auto  md:overflow-x-auto md:w-[70vw] w-[100%] gap-4 items-center  '>
                {path.map((item, index) => (
                    ...
                    <div key={index}>
                    { 
                        item.color1===null ?<div className='flex flex-row items-center justify-center gap-2'> <PathVisualization  station={item.station} type='0' color1={item.color1} color2={item.color2}/> <PiArrowBendDoubleUpRightBold className='text-[30px]  hidden md:block'/> <TbCornerRightDownDouble className='text-[30px]  block md:hidden'/></div>:
                        item.color2===null ?<PathVisualization  station={item.station} type='2' color1={item.color1} color2={item.color2}/>:
                        item.color1!==item.color2 ? <div className='flex flex-row items-center justify-center gap-2'><PathVisualization  station={item.station} type='3' color1={item.color1} color2={item.color2}/> <PiArrowBendDoubleUpRightBold className='text-[30px] hidden md:block'/><TbCornerRightDownDouble className='text-[30px]  block md:hidden'/> </div>:
                        <div className='flex flex-row items-center justify-center gap-2'>   <PathVisualization station={item.station}  type='1' color1={item.color1} color2={item.color2}/> <PiArrowBendDoubleUpRightBold className='text-[30px]  hidden md:block '/> <TbCornerRightDownDouble className='text-[30px]  block md:hidden'/></div>
                    }
